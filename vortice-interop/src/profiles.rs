...
//!
//! It lives in this crate rather than beside one test because three places need exactly the
//! same listener — the TCP interop test, the WebSocket one, and
//! `vortice/examples/regression-listener.rs` — and any drift between them would mean CI
//! verified something other than what a developer runs by hand.
//!
...
///
/// The suite's own listener registers it with the same handler as [`ECHO`]; it is a separate
/// profile only so that a test can have a channel to itself.
const ZERO: &str = "http://iana.org/beep/transient/vortex-regression/zero";
